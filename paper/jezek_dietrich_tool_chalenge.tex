...
Some changes such as removed methods clearly break compatibility, but some breaking changes are less evident. Addition of an method to interface is an obvious example, which breaks compilation of clients implementing this interface. Another less obvious example is making a method more visible. In detail, changing visibility e.g. from \texttt{private} to \texttt{public} may seem harmless, but the new public method may overlap with the same method in a sub-class. When the sub-class method has a stricter access, the compilation also fails as the access cannot be weakened through inheritance.

Since the changes possibly impacting inheritance are partly covered by other categories (method, modifier, types etc. changes), this category adds several examples to the corpus, where class/interface definitions are modified in a sub-class. 
