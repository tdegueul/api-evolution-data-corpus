...
The problem of added abstract/interface methods is addressed in Java 1.8 by providing default methods with the new \texttt{default} keyword. We include such methods in the data set.
...
A change making an element more accessible is usually a compatible change while restricting the access is incompatible. It does not differ for source and binary compatibility. The only known exception is inheritance of a method with a weaker access as already discussed. However, such cases are likely very rare.
