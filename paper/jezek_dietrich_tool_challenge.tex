...
We followed three directions to organize the data and make sure the corpus is complete. 
To build the structure, we used ideas from section 13 of the Java Language Specification, the catalogue by Rivieres \cite{EvolvingJavaAPIs:2007}, work by Dietrich \cite{dietrich14broken} and information found on developer forums.
First, the data set is organised in eight top level categories: access modifiers, data types, exceptions, generics, inheritance, class members, other (non-access) modifiers and miscellaneous.  
Secondly, each category is applicable to a set of seven Java elements: class, inner class, interface, method, constructor, field, generic type. Finally, each combination of a category and an element contains a few possible changes. Basically, changes to an element may be: removed, added, modified. The modifications for datatypes are more complex and will be discussed later in Section \ref{sec:datatypes} in more detail. Modification of other elements is understood as removal of one element and addition of another one. 

Each pair of category and element gave us possibility to construct approximately four changes: one addition, one removal and two more modifications, one strengthening and one weakening. It sums up to about 224 possible combinations ($7*8*4$). However, the number of final test-cases differ slightly due to following reasons: some combinations are not possible (e.g. interface methods may be only public, so no test for access modification is possible). We solved this situation simply by trying all combination for passing Java compilation.  Furthermore, combinations of an element and a category may contain more variants. It particularly holds for method where two cases must be tested: a data-type can be applied either as a parameter or a return type. Furthermore, the data-type may have more then four modifications (boxing, unboxing, primitive/wrapper types, detailed later in Section \ref{sec:datatypes}). This complexity to our best knowledge holds only for data types and we captured all possible combinations. Finally, generics are divided into two more sub-categories containing generic wild-cards and generic parametrised types, producing another dimension. Although generics are relatively complex, capturing possible combinations respects stated modifications. Both wild-card and parametrised types may be parametrised by other types repeating the same modification patter such as addition, removal, etc.

Java also contains a set of constructs that were introduced rather as technical shortcuts and cannot be systematically included into the dataset construction. Example may be implicit inheritance of \texttt{java.lang.Object}. We used mentioned datasets by Rivieres and Dietrich and our experience to add these examples out of order in the corpus. Let us note that we did not include Java annotations in the corpus. The reason is that changes in annotation definitions are the same as changes in the class. Interesting might be changes in annotations application, but annotations serve as meta information that do not directly play role in syntactic compatibility and thus we considered them as out-of-scope. The corpus currently contains 251 scenarios.
...
In summary, the recommended too according to our evaluation is \texttt{sigtest}, which is distributed as open-source and may be easily integrated into development process via CLI, Maven or Ant plugins. Many alternative tools are recommended if only checking of binary compatibility is required. But this may be sufficient in many scenarios as library updates are usually distributed in binary form.  Hence, binary compatibility checking may help to find issues that would otherwise result in runtime failures caused by opaque third-party libraries. Although a source incompatible change may break a system as well, it is detectable during builds early in the process in the development process and therefore less harmful.
