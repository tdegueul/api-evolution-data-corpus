...
\begin{enumerate}
\item[RQ1] Does a tool reliably checking API syntactical compatibility exist? 
\item[RQ2] Does a tool correctly distinguish between source and binary compatibility exist?
\end{enumerate}
...
We followed three directions to organize the data and make sure the corpus is as much complete as possible. The direction targets three axes of \textit{what} can be changed, \textit{where} it applies and \textit{how} it is changed.
...
First, the data set is organised in eight top level \textit{categories}: access modifiers, data types, exceptions, generics, inheritance, class members, other (non-access) modifiers and miscellaneous.  
Secondly, each category may be applicable to a set of seven Java \textit{elements}: class, inner class, interface, method, constructor, field, generic type. Finally, each combination of a category and an element contains a few possible \textit{changes}. Basically, changes to an element may be: a removed element, an added element and a modified element. 
The changes for each pair of category and element most commonly lead to four cases: one for addition, one for removal and two more modifications, one for strengthening and one for weakening. 

It sums up to about 224 possible combinations ($7*8*4$). However, the number of final test-cases differ due to following reasons: some combinations are not possible (e.g. interface methods may be only public, so no test for access modification is possible). We solved this situation simply by trying all combination for passing Java compilation.  In contrast, certain combinations of an element and a category may contain more changes. It particularly holds for a method where two cases must be tested: a data-type can be applied either as a parameter or a return type. Furthermore, the data-type may have more then four modifications (boxing, unboxing, primitive/wrapper types etc., detailed later in Section \ref{sec:datatypes}). This complexity to our best knowledge holds only for data types and we captured all possible combinations. Finally, generics are divided into two more sub-categories containing generic wild-cards and generic parametrised types, producing another dimension. Although generics are relatively complex, capturing possible combinations for evolving API is relatively straightforward.  Both wild-card and parametrised types may be parametrised by other types repeating the same modification patter such as addition, removal, etc.
...
The methodology is centred around Java elements that can build an API, parts of the elements that can be changed and  finally it investigates all changes in each category. Thanks to this design, the produced corpus should be nearly complete nd currently contains 251 scenarios. However, some evolution changes may be missing mainly due to ad-hoc features of Java mentioned above. It makes the design of complete corpus hard, but the corpus may be extended when new evolution changes are discovered. The extension is as simple as adding new source files into the pre-defined directory structure. The detail structure is described in the next section.
...
\paragraph{RQ1 -- Does a tool reliably checking API syntactical compatibility exist}
...
\paragraph{RQ2 -- Does a tool correctly distinguish between source and binary compatibility exist}
