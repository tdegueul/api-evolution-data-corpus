...
%DIF ADD jezek_dietrich_tool_challenge.tex                  Sun Jul  2 17:34:16 2017
%% bare_conf.tex
%% V1.4b
%% 2015/08/26
%% by Michael Shell
...
\DIFdelbegin \DIFdel{The contribution of this paper is twofold}\DIFdelend \DIFaddbegin \DIFadd{Our work addressed the following two research questions:
}
...
\DIFadd{This paper makes two contributions}\DIFaddend . Firstly, we catalogue existing API compatibility checking tools and investigate their capabilities. Secondly, we provide an extensible dataset used for benchmarking such (existing and future) tools. 
...
\DIFadd{Using this terminology, our work targets vertical compatibility as we propose a data set simulating evolution of libraries and then benchmark tools checking API breaking evolution. However, we also worked with horizontal compatibility in order to build an oracle of compatible and incompatible changes. In brief, to detect if an API change is compatible, we developed clients that invoke the APIs. This gave as information about compatibility breaking change for every evolution. 
}\DIFaddend 
...
\DIFaddend In our previous work we have demonstrated how changes to the Java compiler can mitigate certain binary compatibility problems that have been observed \cite{jezek2016dynamo}. \DIFaddbegin \DIFadd{The solution proposed aims to simplify the situation by at least narrowing the gap between source and binary compatibility in Java. However, this requires some rather invasive changes to the Java compiler, and is therefore unlikely to be deployed into standard Java. 
}
...
\DIFadd{Firstly, we have defined eight categories to address the what dimension}\DIFaddend : access modifiers, data types, exceptions, generics, inheritance, class members, other (non-access) modifiers and miscellaneous. 
...
\DIFadd{Secondly, we have searched where those changes can appear and  defined a set of seven Java language elements: class, inner class, interface, method, constructor, field, generic type. 
 }

\DIFadd{Finally, we have investigated how a language element can be changed and defined a few possible change. Basically, a change could may be a removed element, an added element or a modified element. 
...
}\DIFaddend . This is to address the situation that the oracle can be generated by an implementation or version of Java not compatible with the specification.  

The following \DIFdelbegin \DIFdel{sub-sections }\DIFdelend \DIFaddbegin \DIFadd{sub sections }\DIFaddend contain a more detailed discussion of the corpus by category.
...
\DIFdelbeginFL \DIFdelFL{Borderline Cases }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Miscellaneous	 }\DIFaddendFL & 4     & b    	\\ \hline      
\end{tabular}
...
Table \ref{tab:data-overview} shows the number of scenarios in each category and \DIFdelbegin \DIFdel{its }\DIFdelend \DIFaddbegin \DIFadd{their }\DIFaddend typical impact on compatibility (source/binary). \DIFaddbegin \DIFadd{The following sections will discuss changes in each category in more detail, followed with examples to show a typical change in a category.
}\DIFaddend 
...
\item Unbox -- a wrapper type \DIFaddbegin \DIFadd{is }\DIFaddend converted to the matching primitive type
 \end{itemize} 
...
\DIFaddbegin \DIFadd{Examples of possible changes in method parameters follow: 
}
